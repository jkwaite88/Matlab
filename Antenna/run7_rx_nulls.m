...
d_rx = 0.5;
%d_tx = d_rx*N_rx;
% Need to make Tx spacing a bit smaller than theoretical max.
% Due to chebychev weighting.
d_tx = 8;
...
for phi0_i = 1:length(phi0_deg_range),
  ...
  % Get steering (row) vector given power in the main beam direction.
  A_main_rx = exp(j*k0*x_rx*sind(phi0_deg)).';
  
  M = V_null'*(A_main_rx'*A_main_rx)*V_null;
  [Um Sm Vm] = svd(M);

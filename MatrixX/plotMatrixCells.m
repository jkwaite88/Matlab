...
function [binPatches, CartesianLookupTable] = plotMatrixCells(axis_handle, magnitude, useActualBeams)
...
%             right_angles(ind1)];
        ...
        xdata(:,(ind1-1)*(NUM_BINS)+binNum) =...
            (range_values.*cos(angle_values))';
        ydata(:,(ind1-1)*(NUM_BINS)+binNum) =...
        ...
        CartesianLookupTable(ind1,binNum).x = gRange*cosValues(ind1);
        CartesianLookupTable(ind1,binNum).y = gRange*sinValues(ind1);

    end
end
%zdata = -0.0*ones(4,NUM_BEAMS*NUM_BINS);
c = reshape(magnitude,numel(magnitude),1);

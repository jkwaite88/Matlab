...
dataFile = 'C:\Users\jwaite\OneDrive - Wavetronix LLC\Data\DataLogger\2021-04-08\Log_2021-04-08_FalseLoopEventsFlagged.xlsx';
...
if strcmp(fileExt, '.xlsx')
    data.Time = datetime(data.Time, 'ConvertFrom','excel', 'Format','HH:mm:ss.SSS');
    d = data.Time(1);
    data.Time = data.Time - datetime(year(d), month(d), day(d), 0, 0, 0);
    data.Time = duration(data.Time, 'Format', 'hh:mm:ss.SSS');
else
    % create dummy data for files without the falseLoopFlag data
    n = length(data.Time);
    data.FalseLoopFlagZ1 = zeros(n,1);
    data.FalseLoopFlagZ2 = zeros(n,1);
end
...
h = findDistrbutionOfRadarDetectionDurations(data);
...
detectionData.loopFalseCount = zeros(1, numZones);
detectionData.loopFalseTime =  duration(0, 0, 0, 0) * ones(1, numZones);
detectionData.loopFalseTime.Format = 'hh:mm:ss.SSS';
...
%combine data into grouped zones - depending on how many zones the data starts with
combinedData = combineData(data, numZones);

gapLimitSeconds = 0.20;
%remove small gaps between events - (mainly for loops)
...
detectionData = findFalseLoopCalls(cleanedData, detectionData);
...
%     radarPlotFalseSymbols = ['h'; 'p'];
    ...
    for i = 1:numZones
        ...
        loopAx(i) = stairs(fig.ax(i), data.time, data.loop(i).data, 'b', 'linewidth', 1.5);
        symbolPlotIncrement = 0.02;
        symbolPoltValue = 1.00;
        for radarNum = 1:2
            ...
            if numToPlot > 0
                symbolPoltValue = symbolPoltValue + symbolPlotIncrement;
                missedAx(i, radarNum) = plot(detectionData.radarMissedTime(1:numToPlot,i,radarNum), symbolPoltValue*ones(numToPlot,1), 'color', radarPlotColors(radarNum), 'linestyle', 'none', 'marker', radarPlotMissedSymbols(radarNum));
            else
            ...
            if numToPlot > 0
                symbolPoltValue = symbolPoltValue + symbolPlotIncrement;
                falseAx(i, radarNum) = plot(detectionData.radarFalseTime(1:numToPlot,i,radarNum), symbolPoltValue*ones(numToPlot,1), 'color', radarPlotFalseColors(radarNum), 'linestyle', 'none', 'marker', radarPlotFalseSymbols(radarNum));
            else
        ...
        if numToPlot > 0
            symbolPoltValue = symbolPoltValue + symbolPlotIncrement;
            allRadarMisedAx(i) = plot(detectionData.allRadarsMissedTime(ind,i), symbolPoltValue*ones(numToPlot,1), 'color', 'black', 'linestyle', 'none', 'marker', 'd');
        else
        ...
        %plot false loop detects
        ind = find(detectionData.loopFalseTime(:,i) ~= duration(0, 0, 0, 0));
        numToPlot = length(ind);
        if numToPlot > 0
            symbolPoltValue = symbolPoltValue + symbolPlotIncrement;
            loopFalseAx(i) = plot(detectionData.loopFalseTime(ind,i), symbolPoltValue*ones(numToPlot,1), color='b', linestyle='none', marker='s');
        else
            loopFalseAx(i) = plot(duration(0, 0, 0, 0), -0.2, color='b', linestyle='none', marker='s'); % plot one junk marker so we can still have a value for the legend
        end
        
        ylim([0 (symbolPoltValue+symbolPlotIncrement)])
        if i == 1
            title(datestr(date))
            ...
            legend([loopAx(i) rdrAx(i,1) rdrAx(i,2) missedAx(i,1) missedAx(i,2), allRadarMisedAx(1), falseAx(i,1), falseAx(i,2), loopFalseAx(1) xrAx(i), irAx(i)], sprintf('Loop'), sprintf('R1'), sprintf('R2'), sprintf('R1 miss'), sprintf('R2 miss'), sprintf('All miss'), sprintf('R1 false'), sprintf('R2 false'), sprintf('False Loop'), sprintf('XR'), sprintf('IR'), 'location', 'northeast')
        end
...
%this function combines loop and radar data
%after a certain date the radar was changed from 4 zones (two in each direction of travel) to 2 zones (one zone for each direction of travel)
...
function combinedData = combineData(data, numZones)
    ...
    combinedData.falseLoopFlag.zone(1).data = data.FalseLoopFlagZ1;
    combinedData.falseLoopFlag.zone(2).data = data.FalseLoopFlagZ2;
    ...
    if numZones == 1
        ind = find(combinedData.loop(1).data(1:(end-1)) == combinedData.loop(1).data(2:end,1) & ...
                   ...
                   combinedData.falseLoopFlag.zone(1).data(1:(end-1)) ==  combinedData.falseLoopFlag.zone(1).data((2:end)) &...
                   combinedData.falseLoopFlag.zone(2).data(1:(end-1)) ==  combinedData.falseLoopFlag.zone(2).data((2:end)) ...
                   );
    elseif numZones == 2
        ind = find(combinedData.loop(1).data(1:(end-1)) == combinedData.loop(1).data(2:end,1) & ...
    ...
    %when the data was collected each line or time entry indicated that something changed.
       %if multiple loops or zones were combined there is potential for time entries to have no change.
    if ~isempty(ind)
        combinedData.date(ind+1) = [];
        combinedData.time(ind+1) = [];
        combinedData.xr(ind+1) = [];
        combinedData.ir(ind+1) = [];
        combinedData.falseLoopFlag.zone(1).data(ind+1) = [];
        combinedData.falseLoopFlag.zone(2).data(ind+1) = [];
        if numZones == 1
...
function detectionData = findMissedCalls(data, detectionData)
    ...
    for zone = 1:numZones
        ...
        for i = 1:rows
            %debug if statement
            if data.time(i) == duration(02,57,33,712)
                breakhere = 1;
            end
...
%This function identifies the manually identified loop false calls
function detectionData = findFalseLoopCalls(cleanedData, detectionData)
    %simple copy data - this code does no clean up
    n = size(cleanedData.falseLoopFlag.zone, 2);
    for zoneNum = 1:n
        idx = find(cleanedData.falseLoopFlag.zone(zoneNum).data);
        detectionData.loopFalseCount(zoneNum)= length(idx);
        detectionData.loopFalseTime(1:length(idx),zoneNum) = cleanedData.time(idx);
    end
...
function writeReport(detectionData, cleanedData, dataFileName)
    ...
    reportFileName = strcat(filepath, '\', fileName, '_Report', '.txt');
    ...
    %False Loop Detects
    fprintf(reportFileId, 'Loop False Detects\n');
    
    fprintf(reportFileId, '     ');
    fprintf(reportFileId, '   Count\n');
    
    for loop = 1:size(detectionData.loopFalseCount,2)
        fprintf(reportFileId, 'Loop%d\t%d\n', loop, detectionData.loopFalseCount(loop));
    end
...
function h = findDistrbutionOfRadarDetectionDurations(data)
    binEdges = 0:0.1:2;
    for radarNum = 1:2
        for zoneNum = 1:2
            if radarNum == 1
            ...
            numDetections = 0;
            for i = 1:length(d)
                if detectionState == false && d(i) == 1
                    detectionState = true;
                    numDetections = numDetections + 1;
                    startDetectionTime = data.Time(i);
                end
                if detectionState == true && d(i) == 0
                    detectionState = false;
                    delta = data.Time(i) - startDetectionTime;
                    detectionSeconds(numDetections) = seconds(delta);
                end
            end
            subplot(2,2,2*(radarNum-1) + zoneNum)
            h(radarNum,zoneNum) = histogram ((detectionSeconds), binEdges);
            title(sprintf('R%dZ%d',radarNum, zoneNum))
            xlabel('Detection Duration (seconds)')
        end
    end
end
